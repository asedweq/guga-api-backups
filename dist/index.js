...
const REPORT_TRACES = node_process_1.default.env.CSG_REPORT_TRACES === "1";
const REPORT_PROXY_KEY = node_process_1.default.env.CSG_PROXY_KEY ?? "";
function newTraceId() {
    return node_crypto_1.default.randomBytes(16).toString("hex");
}
// ─── Constants ───────────────────────────────────────────────────────────────
const PORT = parseInt(node_process_1.default.env.PORT ?? "3000", 10);
...
const NODE_PROTOCOL = (node_process_1.default.env.NODE_PROTOCOL ?? "https").replace(/:?\/*$/, "");
...
function log(level, msg, extra) {
    ...
    if (level === "ERROR")
        node_process_1.default.stderr.write(line + "\n");
    else
        node_process_1.default.stdout.write(line + "\n");
}
function makeTraceCollector(traceId) {
    const events = [];
    function emit(level, msg, extra) {
        const ev = { t: Date.now(), level, msg, traceId, ...(extra ?? {}) };
        events.push(ev);
...
async function handleRequest(req, res) {
    ...
    if (BODY_METHODS.has(method)) {
        try {
            ...
            sendError(res, 400, "body_read_error", String(err?.message ?? err));
            return;
        }
    }
    // ── Session lifecycle ──────────────────────────────────────────────────────
...
node_process_1.default.on("SIGTERM", () => {
    ...
    server.close(() => node_process_1.default.exit(0));
    setTimeout(() => node_process_1.default.exit(1), 10_000);
});
